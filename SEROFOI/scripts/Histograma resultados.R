...
sero_rev_VPH16 <- ggplot(VPH16) +
  geom_histogram(aes(x = `Seroreversión`), fill = "#1f77b4", color = "black") +
  labs(y = "Frecuencia",title = "VPH-16", x = "Serorreversión" ) +
  ylim(0, 2)
...
sero_rev_VPH18 <- ggplot(VPH16) +
  geom_histogram(aes(x = `Seroreversión`), fill = "#FF5733", color = "black") +
  labs(y = "Frecuencia",title = "VPH-16", x = "Serorreversión" ) +
  ylim(0, 2)
...
VPH16$Genotipo <- "VPH-16"
VPH18$Genotipo <- "VPH-18"
...
FOI_combined_plot <- ggplot(VPH_combined, aes(x = País, y = FOI, fill = Genotipo)) + 
  ...
  labs(x = "País", y = "Fuerza de Infección (per cápita)") +
  theme_minimal() +
  theme(
  ...
  ylim(0, 1.5) +
  scale_fill_manual(values = c("VPH-16" = "#007BFF", "VPH-18" = "#FF5733"))
